...
const ProductCard = ({ item }) => {
    ...
    return (
        <div className="bg-black text-white rounded-lg shadow-md overflow-hidden border border-gray-700 p-2">
            <Link to={`/product/${item.id}`}>
                <img
                    src={item.images[0]}
                    alt={item.title}
                    className="w-full h-48 object-contain bg-black p-1"
                />
            </Link>
            <div className="p-4">
...
const Sidebar = ({ setSort, filters, setFilters, priceRange, setPriceRange }) => {
    ...
    return (
        <div className="bg-black text-white border-r border-gray-1000 w-64 px-6 py-4 space-y-6">

            {/* Sort */}
            <div>
                <h3 className="text-lg font-bold mb-2">Sort By</h3>
                <ul className="space-y-1 text-gray-300 cursor-pointer">
                    <li onClick={() => setSort("latest")} className="sideBarHover">Latest</li>
                    <li onClick={() => setSort("featured")} className="sideBarHover">Featured</li>
                    <li onClick={() => setSort("topRated")} className="sideBarHover">Top Rated</li>
                    <li onClick={() => setSort("lowToHigh")} className="sideBarHover">Price (Lowest First)</li>
                    <li onClick={() => setSort("highToLow")} className="sideBarHover">Price (Highest First)</li>
                </ul>
            </div>
        ...
        <div className="bg-black min-h-screen text-white allprodctsdisplay flex">
            ...
            <div className="flex-1 p-6">
                ...
                {filteredProducts.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 pt-10 pb-10">
